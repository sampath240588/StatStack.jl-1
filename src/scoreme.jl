...
# ------------------------------------------------------ SCORING -------------------------------------------------------------------
# ----------------------------------------------------------------------------------------------------------------------------------
#  pp=readtable("/mapr/mapr04p/analytics0001/analytic_users/Models/pid.csv")
#  p=setdiff(df_data[:panid],pp[:panid])
#  df_data2=df_data[findin(df_data[:panid],p),:]
#  length(df_data[:panid])-length(df_data2[:panid])
# df_data=df_data2
# ----------------------------------------------------------------------------------------------------------------------------------
...
# 00000 test 00000
##"/mapr/mapr04p/analytics0001/analytic_users/Models/Hormel_spam_1067/Spam1067_model/output/Scoring"
##r = genMDF(cnts, mocc, mdolocc, mpen)
##r[(r[:key].=="creativename (Can Banner)"),:]
##r[(r[:key].=="Total Campaign"),:]
##rmdf=genRndMDF(cnts, mocc, mdolocc, mpen)
##rmdf[(rmdf[:key].=="creativename (Can Banner)"),:]
